...
CFG="${1:-/etc/gateway/gatewayd.json}"
...
# Build the binary in the canonical place this project uses.
...
trap 'echo; echo "Stopping..."; [[ -n "${GW_PID:-}" ]] && kill "${GW_PID}" >/dev/null 2>&1 || true; rm -rf "$TMPDIR"' EXIT

# Create a temp config that:
# - keeps your config structure,
# - adds demo_health_file for gpio_stub endpoints,
# - DISABLES all but one mcptt_stub endpoint so there is no preempt/competition,
# - forces arbitration policy to first_wins (deterministic for this test).
...
# Find enabled mcptt_stub endpoints
...
# Keep only the first enabled mcptt_stub; disable the rest (prevents preempt/competition).
keep_id = mcptt[0].get("id")
for e in mcptt[1:]:
    e["enabled"] = False

# Attach demo_health_file to all enabled gpio_stub endpoints (lets test flip health at runtime).
...
# Force arbitration to deterministic first_wins for this test.
...
# Print info for bash.
print(keep_id)               # mcptt id in use
print(gpio[0]["id"])         # dest1
print(gpio[1]["id"])         # dest2
PY
)
...
# Initial health: DEST1 healthy, DEST2 down => TXStart must choose DEST1.
echo "healthy" > "$H1"
echo "down"    > "$H2"
        ...
        with open(logfile, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                if o.get("event_type") == event_type and o.get("msg") == "event_received":
                    return True
...
echo
echo "SUCCESS: destination locking verified under mid-TX health flip."
